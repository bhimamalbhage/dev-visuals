...
ON Orders.EmployeeID = Employees.EmployeeID;`,
        use_case:
          "When you want all records from the second table regardless of matches",
      },
      {
        id: "full",
...
ON Customers.CustomerID = Orders.CustomerID;`,
        use_case:
          "When you want to see all records from both tables regardless of matches",
      },
      {
        id: "self",
        name: "SELF JOIN",
...
CROSS JOIN Categories;`,
            ...
            {joins.slice(0, 6).map((join) => (
              ...
              >
                ...
                <svg width="120" height="80" viewBox="0 0 150 100">
                  ...
                  {join.id === "left" && (
                    <path
                      d="M60,10 A40,40 0 0 1 60,90 A40,40 0 0 1 60,10 Z"
                      fill="#2563eb"
                      fillOpacity="0.7"
                    />
                  )}
                  ...
                  {join.id === "right" && (
                    <path
                      d="M90,10 A40,40 0 0 0 90,90 A40,40 0 0 0 90,10 Z"
                      fill="#2563eb"
                      fillOpacity="0.7"
                    />
                  )}
                  ...
                  {join.id === "full" && (
                    <path
                      d="M60,10 A40,40 0 0 1 60,90 A40,40 0 0 1 60,10 Z M90,10 A40,40 0 0 0 90,90 A40,40 0 0 0 90,10 Z"
                      fill="#2563eb"
                      fillOpacity="0.7"
                    />
                  )}
          ...
          <div className="flex-grow bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
              <div className="space-y-4">
                <h4 className="font-bold text-lg text-blue-700 border-b border-blue-200 pb-2">
                  {selectedJoin.name}
                </h4>
                <div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-yellow-50 p-3 rounded border border-yellow-200">
                  ...
                  <p className="text-yellow-700 text-sm mt-1">
                    {selectedJoin.id === "inner" &&
                      "INNER JOIN is the most commonly used join and has the best performance. Use when you only need matching records."}
                    {selectedJoin.id === "left" &&
                      "Always check for NULL values in the result set when using LEFT JOIN to identify records that don't have matches."}
                    {selectedJoin.id === "right" &&
                      "RIGHT JOIN is less common; the same result can often be achieved by swapping tables and using LEFT JOIN."}
                    {selectedJoin.id === "full" &&
                      "FULL OUTER JOIN can be resource-intensive. Consider if you really need all records from both tables."}
                    {selectedJoin.id === "self" &&
                      "Self joins are powerful for hierarchical data like employee-manager relationships or category trees."}
                    {selectedJoin.id === "cross" &&
                      "Use CROSS JOIN with caution as it produces a row for every combination of rows in the tables, which can be very large."}
                  </p>
                </div>
              </div>
            </div>
    ...
    const content = {
      select: {
        ...
        examples: [
          ...
          {
            name: "Select with Calculation",
            code: "SELECT ProductName, Price * Quantity AS Total FROM OrderDetails;",
          },
        ],
      },
      where: {
        ...
        examples: [
          ...
          {
            name: "Greater Than",
            code: "SELECT * FROM Products WHERE Price > 30;",
          },
          {
            name: "Between Range",
            code: "SELECT * FROM Products WHERE Price BETWEEN 50 AND 60;",
          },
          {
            name: "Pattern Matching",
            code: "SELECT * FROM Customers WHERE City LIKE 'L%';",
          },
          {
      ...
      groupby: {
        ...
        examples: [
          ...
          {
            name: "Sum by Group",
            code: "SELECT ProductID, SUM(Quantity) AS TotalOrdered FROM OrderDetails GROUP BY ProductID;",
          },
          {
            name: "Multiple Columns",
            code: "SELECT ShipperID, ProductID, SUM(Quantity) FROM Orders GROUP BY ShipperID, ProductID;",
          },
        ],
      },
      having: {
        ...
        examples: [
          ...
          {
            name: "With Sum",
            code: "SELECT ProductID, SUM(Quantity) AS Total FROM OrderDetails GROUP BY ProductID HAVING SUM(Quantity) > 100;",
          },
        ],
      },
      orderby: {
        ...
        examples: [
          {
            ...
            code: "SELECT * FROM Customers ORDER BY Country;",
          },
          {
            name: "Descending Order",
            code: "SELECT * FROM Customers ORDER BY Country DESC;",
          },
          {
            name: "Multiple Columns",
        ...
        <div className="flex space-x-2 mb-6">
          {sections.map((section) => (
            <button
              ...
              style={
                activeSection === section.id
                  ? { backgroundColor: getColor(section.color) }
                  : {}
              }
            >
              {section.name}
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
          ...
          <div className="mt-6 bg-gray-50 p-4 rounded-md border border-gray-200">
            ...
            <ol className="list-decimal list-inside text-sm text-gray-600 space-y-1">
              <li
                className={
                  activeSection === "select" ? "font-bold text-blue-700" : ""
                }
              >
              ...
              <li
                className={
                  activeSection === "where" ? "font-bold text-green-700" : ""
                }
              >
                WHERE - Filters rows
              </li>
              <li
                className={
                  activeSection === "groupby" ? "font-bold text-purple-700" : ""
                }
              >
              ...
              <li
                className={
                  activeSection === "having" ? "font-bold text-orange-700" : ""
                }
              >
              ...
              <li
                className={
                  activeSection === "orderby" ? "font-bold text-pink-700" : ""
                }
              >
    ...
    const dataTypeCategories = [
      {
        ...
        types: [
          ...
          },
          {
            ...
            example: "3.14159",
            range: "±1.79E+308 (varies by database)",
          },
          {
            name: "REAL",
            description: "Single precision floating point",
            example: "3.14",
            range: "±3.40E+38 (varies by database)",
          },
        ],
      },
      {
        ...
        types: [
          {
            ...
            example: "'text'",
            range: "1 to 255 characters (varies)",
          },
          {
            name: "VARCHAR(n)",
            description: "Variable-length character string",
            example: "'text'",
            range: "1 to 65,535 characters (varies)",
          },
          {
            name: "TEXT",
            description: "Variable-length character string",
            example: "'long text...'",
            range: "Up to 65,535 characters (varies)",
          },
          {
            name: "NVARCHAR(n)",
            description: "Variable-length Unicode string",
            example: "N'text'",
            range: "1 to 4,000 characters (varies)",
          },
          {
            name: "CLOB",
            description: "Character Large Object",
            example: "Very long text...",
            range: "Up to 2GB (varies)",
          },
        ],
      },
      {
        ...
        types: [
          {
            ...
            example: "'2023-04-13'",
            range: "1000-01-01 to 9999-12-31 (varies)",
          },
          {
            name: "TIME",
          ...
          },
          {
            ...
            example: "'2023-04-13 13:45:30'",
            range: "Varies by database",
          },
          {
            name: "TIMESTAMP",
            description: "Date and time with time zone",
            example: "'2023-04-13 13:45:30+00'",
            range: "Varies by database",
          },
          {
            name: "INTERVAL",
        ...
        types: [
          {
            ...
            example: "0x1234",
            range: "1 to 255 bytes (varies)",
          },
          {
            name: "VARBINARY(n)",
            description: "Variable-length binary data",
            example: "0x1234",
            range: "1 to 65,535 bytes (varies)",
          },
          {
            name: "BLOB",
            description: "Binary Large Object",
            example: "Binary data...",
            range: "Up to 4GB (varies)",
          },
        ],
      },
      {
      ...
      },
      {
        ...
        types: [
          ...
          },
          {
            ...
            example: "<person><n>John</n></person>",
            range: "Varies by database",
          },
          {
            name: "UUID",
          ...
          {
            name: "ENUM",
            description: "Enumeration of strings",
            example: "ENUM('small', 'medium', 'large')",
            range: "Limited by definition",
          },
        ],
      },
    ];
            ...
            <ul className="list-disc list-inside text-sm text-blue-700 space-y-1">
              ...
              {activeCategory === "String Types" && (
                <>
                  ...
                  <li>
                    Use TEXT for very long strings without specific length
                  </li>
                </>
              )}
              {activeCategory === "Date and Time Types" && (
                <>
                  ...
                  <li>
                    Use TIMESTAMP for event logging with time zone awareness
                  </li>
                  <li>
              ...
              {activeCategory === "Binary Types" && (
                <>
                  <li>
                    Use BLOB for storing images, documents, or other binary
                    files
                  </li>
              ...
              {activeCategory === "Other Types" && (
                <>
                  ...
                  <li>
                    ENUM provides type safety but can be difficult to alter
                    later
                  </li>
                </>
              )}
    ...
    const indexTypes = [
      {
        name: "B-Tree Index",
        description:
          "The default index type in most databases. Balanced tree structure for efficient lookups, range queries, and sorting.",
        benefits: [
          "Good for equality (=) and range (>, <, BETWEEN) searches",
          "Supports sorting operations (ORDER BY)",
          "Works well with high-cardinality columns",
        ],
        limitations: [
          "Less efficient for low-cardinality columns",
          "Not ideal for full text searches",
          "Storage overhead increases with index size",
        ],
      ...
      },
      {
        name: "Unique Index",
        description:
          "Ensures all values in the indexed column(s) are unique. Prevents duplicate values in the column(s).",
        benefits: [
          "Enforces data integrity",
          "Improves query performance",
          "Can be used to implement primary and unique keys",
        ],
        limitations: [
          "Cannot insert duplicate values",
          "Slightly more overhead than non-unique indexes",
        ],
      ...
      },
      {
        ...
        description:
          "An index on multiple columns. The order of columns is important for query optimization.",
        benefits: [
          "Useful for queries that filter on multiple columns",
          "Can improve performance of JOIN operations",
          "Can cover more queries with fewer indexes",
        ],
        limitations: [
          "Only effective when leading columns are in the filter",
          "More maintenance overhead than single-column indexes",
        ],
        example:
          "CREATE INDEX idx_last_first ON Employees(LastName, FirstName);",
      },
      {
        name: "Full-Text Index",
        description:
          "Specialized index type designed for text searching in documents or large text fields.",
        benefits: [
          "Enables efficient text searches",
          "Supports linguistic matching",
          "Can handle stemming, synonyms, and stop words",
        ],
        limitations: [
          "Higher storage and maintenance overhead",
          "Not available in all databases",
          "May not perform well for non-text operations",
        ],
        example:
      ...
      },
      {
        name: "Hash Index",
        description:
          "Uses a hash function to map key values to index entries. Very fast for exact match lookups.",
        benefits: [
          "Extremely fast for equality searches",
          "Constant-time lookup performance",
          "Compact storage for some implementations",
        ],
        limitations: [
          "Cannot support range-based queries",
          "No sorting capability",
          "Not efficient for partial matches (LIKE)",
        ],
        example:
          "CREATE INDEX idx_customer_id ON Customers USING HASH (CustomerID);",
      },
    ];
          ...
          {indexTypes.map((indexType) => (
            <button
              ...
              className={`px-3 py-2 rounded-md text-sm ${
                selectedIndex.name === indexType.name
                  ? "bg-blue-600 text-white"
    ...
    const aggregations = [
      {
        ...
        examples: [
          ...
          {
            label: "Count non-NULL values",
            code: "SELECT COUNT(City) FROM Customers;",
          },
          {
            label: "Count with grouping",
            code: "SELECT Country, COUNT(*) FROM Customers GROUP BY Country;",
          },
          {
      ...
      },
      {
        ...
        examples: [
          ...
          {
            label: "Sum with condition",
            code: "SELECT SUM(Price) FROM Products WHERE CategoryID = 1;",
          },
          {
            label: "Sum with calculation",
            code: "SELECT SUM(Price * Quantity) FROM OrderDetails;",
          },
          {
            label: "Sum by group",
            code: "SELECT ProductID, SUM(Quantity) FROM OrderDetails GROUP BY ProductID;",
          },
        ],
      },
      {
        ...
        examples: [
          ...
          {
            label: "Average with filters",
            code: "SELECT AVG(Price) FROM Products WHERE CategoryID = 2;",
          },
          {
            label: "Average by group",
            code: "SELECT CategoryID, AVG(Price) FROM Products GROUP BY CategoryID;",
          },
          {
            label: "Average with HAVING",
            code: "SELECT CategoryID, AVG(Price) FROM Products GROUP BY CategoryID HAVING AVG(Price) > 20;",
          },
        ],
      },
      {
        ...
        examples: [
          ...
          {
            label: "Min/Max with grouping",
            code: "SELECT CategoryID, MIN(Price), MAX(Price) FROM Products GROUP BY CategoryID;",
          },
          {
            label: "Find record with max value",
            code: "SELECT * FROM Products WHERE Price = (SELECT MAX(Price) FROM Products);",
          },
        ],
      },
      {
        ...
        examples: [
          {
            label: "Basic concatenation",
            code: "SELECT CategoryID, GROUP_CONCAT(ProductName) FROM Products GROUP BY CategoryID;",
          },
          {
            label: "With separator",
            code: "SELECT CategoryID, GROUP_CONCAT(ProductName SEPARATOR ', ') FROM Products GROUP BY CategoryID;",
          },
          {
            label: "With ordering",
            code: "SELECT CategoryID, GROUP_CONCAT(ProductName ORDER BY Price DESC) FROM Products GROUP BY CategoryID;",
          },
          {
            label: "With DISTINCT",
            code: "SELECT GROUP_CONCAT(DISTINCT City ORDER BY City) FROM Customers;",
          },
        ],
      },
    ];
              ...
              <ul className="list-disc pl-5 space-y-1 text-blue-800 text-sm">
                ...
                {selectedAgg.name === "MIN / MAX" && (
                  <>
                    ...
                    <li>
                      Can be used in subqueries to find records with extreme
                      values
                    </li>
                  </>
                )}
                {selectedAgg.name === "GROUP_CONCAT" && (
                  <>
                    ...
                    <li>
                      SQL Server uses STRING_AGG(), PostgreSQL uses STRING_AGG()
                      or ARRAY_AGG()
                    </li>
              ...
              <ul className="list-disc pl-5 text-sm text-yellow-800 space-y-1">
                {selectedAgg.name === "COUNT" && (
                  <>
                    ...
                    <li>
                      Using COUNT in a SELECT without GROUP BY returns only one
                      row
                    </li>
                ...
                {selectedAgg.name === "SUM" && (
                  <>
                    <li>
                      NULL values are ignored, which may lead to unexpected
                      results
                    </li>
                ...
                {selectedAgg.name === "AVG" && (
                  <>
                    ...
                    <li>
                      Consider using weighted averages for some calculations
                    </li>
                  </>
                )}
                {selectedAgg.name === "MIN / MAX" && (
                  <>
                    ...
                    <li>
                      Date formats must be consistent for proper comparisons
                    </li>
                ...
                {selectedAgg.name === "GROUP_CONCAT" && (
                  <>
                    <li>
                      Default output length may be limited (adjust
                      group_concat_max_len)
                    </li>
                    <li>
                      Order of concatenation not guaranteed without ORDER BY
                    </li>
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      {/* Sidebar */}
      <div
        className={`w-64 bg-white shadow-md z-40 transition-all ${
      ...
      <div className="flex-1 p-4 md:p-6 overflow-auto">
        ...
        {/* Diagram display */}
        <div className="mt-4">{renderDiagram()}</div>
      </div>
    </div>
  );
};
